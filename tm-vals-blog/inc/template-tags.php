...
if ( ! function_exists( 'tm_vals_blog_posted_on' ) ) :
/**
...
if ( ! function_exists( 'tm_vals_blog_entry_footer' ) ) :
/**
 ...
 * @return bool
 */
function tm_vals_blog_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'tm_vals_blog_categories' ) ) ) {
		...
		set_transient( 'tm_vals_blog_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so tm_vals_blog_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so tm_vals_blog_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in tm_vals_blog_categorized_blog.
 */
function tm_vals_blog_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'tm_vals_blog_categories' );
}
add_action( 'edit_category', 'tm_vals_blog_category_transient_flusher' );
add_action( 'save_post',     'tm_vals_blog_category_transient_flusher' );
